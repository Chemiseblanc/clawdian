...
export interface OmpForkSource {
  resumeAt: string;
  sessionId: string;
}

export interface OmpPreviousSession {
  leafEntryId?: string;
  sessionFile?: string;
  sessionId?: string;
}

export interface OmpProviderState {
  forkSource?: OmpForkSource;
  forkSourceSessionFile?: string;
  leafEntryId?: string;
  parentSession?: string;
  previousSessions?: OmpPreviousSession[];
  sessionFile?: string;
  sessionId?: string;
}

export function getOmpState(value: unknown): OmpProviderState {
  if (!value || typeof value !== 'object' || Array.isArray(value)) {
    return {};
  }

  const record = value as Record<string, unknown>;
  const forkSource = getOmpForkSource(record.forkSource);
  const previousSessions = getOmpPreviousSessions(record.previousSessions);
  return {
...
export function buildPersistedOmpState(state: OmpProviderState): OmpProviderState | undefined {
  const persisted: OmpProviderState = {
...
export function clearOmpResumeState(conversation: Conversation): boolean {
  ...
  const previousSessions = addOmpPreviousSession(
...
function getOmpForkSource(value: unknown): OmpForkSource | undefined {
  if (!value || typeof value !== 'object' || Array.isArray(value)) {
    return undefined;
  }
...
function getOmpPreviousSessions(value: unknown): OmpPreviousSession[] {
  if (!Array.isArray(value)) {
    return [];
  }

  return value.flatMap((item): OmpPreviousSession[] => {
...
export function addOmpPreviousSession(
  sessions: readonly OmpPreviousSession[] | undefined,
  candidate: OmpPreviousSession,
): OmpPreviousSession[] {
