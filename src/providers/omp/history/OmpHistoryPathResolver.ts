...
function getTrustedRoots(
  ...
  const profile = (context.environment.OMP_PROFILE
    ?? context.environment.PI_PROFILE
    ?? '').trim();
  const configuredAgentDir = context.environment.PI_CODING_AGENT_DIR?.trim();
  if (!profile && configuredAgentDir && path.isAbsolute(configuredAgentDir)) {
  ...
  if (profile) {
    roots.push(path.join(home, configDir, 'profiles', profile, 'agent', 'sessions'));
  }
  roots.push(path.join(home, configDir, 'agent', 'sessions'));
