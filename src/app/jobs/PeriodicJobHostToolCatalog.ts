...
import type { PeriodicJobDraft, StoredChatModelSelection } from '../../core/types';

export const PERIODIC_JOB_HOST_TOOL_NAMES = Object.freeze({
  list: 'claudian.periodic_job.list',
  create: 'claudian.periodic_job.create',
  update: 'claudian.periodic_job.update',
  delete: 'claudian.periodic_job.delete',
} as const);
...
const DEFINITIONS: readonly HostToolDefinition[] = Object.freeze([
  Object.freeze({
    name: PERIODIC_JOB_HOST_TOOL_NAMES.list,
  ...
  Object.freeze({
    name: PERIODIC_JOB_HOST_TOOL_NAMES.create,
  ...
  Object.freeze({
    name: PERIODIC_JOB_HOST_TOOL_NAMES.update,
  ...
  Object.freeze({
    name: PERIODIC_JOB_HOST_TOOL_NAMES.delete,
...
export class PeriodicJobHostToolCatalog implements HostToolCatalog {
  constructor(private readonly jobs: PeriodicJobsHostPort) {}
  ...
  async invoke(
    ...
    try {
      switch (name) {
        case PERIODIC_JOB_HOST_TOOL_NAMES.list:
          requireExactObject(input, EMPTY_KEYS);
          return {
            ok: true,
            value: {
              jobs: this.jobs.list().map(job => ({
                ...structuredClone(job),
                running: this.jobs.isRunning(job.id),
              })),
            },
          };
        case PERIODIC_JOB_HOST_TOOL_NAMES.create: {
          const value = requireExactObject(input, CREATE_KEYS);
          const draft = parseCreateInput(value, context);
          return { ok: true, value: { job: await this.jobs.create(draft) } };
        }
        case PERIODIC_JOB_HOST_TOOL_NAMES.update: {
          ...
          return { ok: true, value: { job: await this.jobs.updatePartial(id, patch) } };
        }
        case PERIODIC_JOB_HOST_TOOL_NAMES.delete: {
...
function mapError(error: unknown): HostToolResult {
  ...
  if (!(error instanceof Error)) {
    return failure('internal_error', 'Periodic job operation failed.');
  }
  const code = errorCodeForMessage(error.message);
  return code
    ? failure(code, error.message)
    : failure('internal_error', 'Periodic job operation failed.');
}
