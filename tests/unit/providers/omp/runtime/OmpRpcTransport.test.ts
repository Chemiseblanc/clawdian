...
import {
  OmpRpcResponseError,
  OmpRpcTransport,
  OmpRpcTransportClosedError,
} from '@/providers/omp/runtime/OmpRpcTransport';
...
function createTransport() {
  ...
  const transport = new OmpRpcTransport({ input, output }, 100);
  transport.start();
...
describe('OmpRpcTransport', () => {
  ...
  it('unwraps Omp response data when result is absent', async () => {
    const { input, transport } = createTransport();
  ...
  it('rejects failed responses and pending requests on dispose', async () => {
    ...
    await expect(failed).rejects.toBeInstanceOf(OmpRpcResponseError);

    const pending = transport.request('prompt');
    transport.dispose();
    await expect(pending).rejects.toBeInstanceOf(OmpRpcTransportClosedError);
  });
