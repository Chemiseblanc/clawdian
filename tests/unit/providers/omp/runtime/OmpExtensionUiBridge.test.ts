...
import type { OmpRpcTransport } from '@/providers/omp/runtime/OmpRpcTransport';

function createBridge(renderer: Partial<OmpExtensionUiRenderer>) {
  const transport = {
    send: jest.fn(),
  } as unknown as OmpRpcTransport;
  const bridge = new OmpExtensionUiBridge(transport, renderer as OmpExtensionUiRenderer);
