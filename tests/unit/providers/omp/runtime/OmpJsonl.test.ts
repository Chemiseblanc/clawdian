...
import { subscribeOmpJsonlLines, writeOmpJsonl } from '@/providers/omp/runtime/OmpJsonl';

describe('OmpJsonl', () => {
  it('splits only on LF and strips CR', () => {
    const stream = new PassThrough();
    const lines: string[] = [];
    subscribeOmpJsonlLines(stream, line => lines.push(line));
  ...
  it('writes JSONL records', () => {
    ...
    writeOmpJsonl(output, { type: 'ping' });
