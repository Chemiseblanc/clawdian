...
describe('OmpSubprocess', () => {
  ...
  it('spawns Omp RPC with the launch spec args, cwd, stdio, and enhanced PATH', () => {
    ...
    expect(mockSpawn).toHaveBeenCalledWith('/opt/omp/bin/omp', ['--mode', 'rpc'], expect.objectContaining({
      cwd: '/vault',
      stdio: 'pipe',
      windowsHide: true,
  ...
  it('wraps Windows .cmd shims through cmd.exe', () => {
    ...
    expect(mockSpawn).toHaveBeenCalledWith(
      ...
      ['/d', '/s', '/c', '""C:\\Users\\R&D\\AppData\\Roaming\\npm\\omp.cmd" --mode rpc --system-prompt "Use R&D policy""'],
